...
import { tomorrow } from './_apiKeys.mjs'
...
export const getTimelines = async (lat, lon, startTime, endTime, timezone = 'UTC', format = 'fire') => {
  ...
  try {
    ...
    if (res.status === 429) { // 'Too Many Requests' 3/sec, 25/hr, 500/day
      ...
      const records = (format === 'fire') ? asFireWeather(jsonResponse) : asTomorrow(jsonResponse)
      return records
    }
    // if ( typeof jsonResponse.data === 'undefined' ) {
    //   console.error('getTimelines()', JSON.stringify(json))
    //   return []
    // }
  } catch (error) {
